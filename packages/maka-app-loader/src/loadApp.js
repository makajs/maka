...
function getUrl(app) {
  if (typeof app === 'string') {
    ...
    if (config.current.appUrls) {
      const ret = config.current.appUrls[app];
      if (ret) {
        return fixUrl(ret);
      }
    ...
    if (app.url) { return fixUrl(app.url); }
...
function findNameByUrl(url) {
  let ret = '';
  if (config.current.appUrls) {
    const hit = Object.keys(config.current.appUrls).find(k => config.current.appUrls[k].url === url);
    ret = hit;
  }
  if (ret) return ret;
...
export default function loadApp(app) {
  ...
  return new Promise(resolve => {
    urls = urls.filter(url => {
      const appName = findNameByUrl(url);
      const pub = url.indexOf('/') ? url.substr(0, url.lastIndexOf('/') + 1) : '';
    ...
    globalObj.require(urls, async (...args) => {
      ...
      if (!apps) {
        ...
        const cssUrls = urls.map(u => {
          if (u.indexOf('http') !== -1) { return `css!${u.replace('.js', '.css')}`; }
          return `css!${u}`;
        });
        /*
