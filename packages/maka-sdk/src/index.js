...
const createAppElementInternal = (appName, appProps) => props => {
  return (
    <Provider store={store}>
      <appLoader.AppLoader name={appName} {...appProps} {...props} />
    </Provider>
  );
};
function createAppElement(appName, appProps) {
  const Internal = createAppElementInternal(appName, appProps);
...
async function render(appName, targetDomId, disableRoute, props = {}) {
  const appNameNoQuery = appName.split('?')[0];
  if (!appLoader.existsApp(appNameNoQuery)) { await appLoader.loadApp(appNameNoQuery, isProduction); }
  ...
  if (disableRoute) {
    if (Hoc) {
      domRender((
        <Hoc>
          <Provider store={store}>
            <appLoader.AppLoader name={appName} {...props} />
          </Provider>
        </Hoc>
