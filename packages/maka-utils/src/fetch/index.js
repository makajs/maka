...
function mock(url, handler) {
  ...
  if (url && typeof url === 'object') {
    ...
    Object.keys(url).forEach(u => mock(u, url[u]));
    ...
    const pre = paths.shift();
    Object.keys(handler).forEach(key => {
      const theUrl = pre + key + paths.join('*');
...
function fixUrl(url) {
  if (_options.prefix) { return `${_options.prefix}${url}`; }
  return url;
}
...
function formPost(url, data, isFree) {
  data = data || {};
  const accessToken = getAccessToken();// toke in sessionStorage
  if (!!accessToken && !isFree) {
    data.token = accessToken;
  }

  const postForm = document.createElement('form');// form object
