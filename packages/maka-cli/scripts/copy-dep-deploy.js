...
depPaths.forEach(p => {
  const buildPath = path.resolve(p[0], 'build', isDev ? 'dev' : 'prod');

  if (fs.existsSync(buildPath)) {
    const target = `${targetPath}/${p[1]}/v${version ? version : p[2]}`;
  ...
  files.forEach(fileName => {
    ...
    if (stats.isFile()) {
      if (fileName === 'package.json') {
        const subAppJson = require(path.join(dir, 'package.json'));
        if (subAppJson.isMakaApp) {
          const appPath = path.relative(paths.appPath, dir);
...
function scanRemoteApps(json) {
  ...
  Object.keys(json.dependencies).forEach(k => {
    const filePath = path.resolve(paths.appSrc, 'node_modules', k, 'package.json');
    if (fs.existsSync(filePath)) {
      const pkg = JSON.parse(fs.readFileSync(path.resolve(paths.appSrc, 'node_modules', k, 'package.json'), 'utf-8'));
      if (pkg.isMakaApp) {
        const appPath = path.join(paths.appSrc, 'node_modules', k);
        if (depPaths.indexOf(appPath) === -1) {
