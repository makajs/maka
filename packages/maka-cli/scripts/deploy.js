...
function copyCoreLib() {
    ...
    let ver = version
    let libPath = path.resolve(__dirname, '..', 'node_modules', '@makajs', 'sdk', 'dist', isDev ? 'debug' : 'release')
    if (!ver) {

        let packageJson = require(path.resolve(__dirname, '..', 'node_modules', '@makajs', 'sdk', 'package.json'))
        ver = packageJson.version
    }

    let target = `${outputPath}/core/v${ver}/`
    coreVersion = ver
    coreDirectory = target

    if (!fs.existsSync(`${outputPath}/core`))
        fs.mkdirSync(`${outputPath}/core`)
